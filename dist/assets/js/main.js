let preview=document.getElementById("preview"),previewImg=document.createElement("img");function dragNdrop(e){e=URL.createObjectURL(e.target.files[0]);previewImg.setAttribute("src",e),document.querySelector(".post_img").setAttribute("src",e),preview.innerHTML="",preview.appendChild(previewImg)}function drag(){document.getElementById("uploadFile").parentNode.className="draging dragBox"}function drop(){document.getElementById("uploadFile").parentNode.className="dragBox"}document.querySelectorAll(".add_post_link a").forEach(function(e){e.addEventListener("click",function(e){e.preventDefault();e=e.target.id;if(console.log(e),"post-pic"===e)document.querySelector(".uploadOuter").style.display="block",document.querySelector(".post_img").style.display="block";else if("post-felling"===e)fetchingApi();else if("post-video"===e){document.getElementById("webCam").style.display="block";let t=document.getElementById("vid");e=navigator.mediaDevices;vid.muted=!0,e.getUserMedia({video:!0,audio:!0}).then(e=>{t.srcObject=e,t.addEventListener("loadedmetadata",()=>{t.play()})}).catch(alert)}})}),document.getElementById("button_value").onclick=()=>{var e=document.getElementById("input_value").value,t=(document.getElementById("preview").style.display="none",document.querySelector(".uploadOuter").style.display="none",document.createElement("div"));t.classList.add("add_post_container"),t.style.display="block",t.innerHTML=`
        <div class="post_row">
